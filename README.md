...
<h1 align="center">Gald3r for OpenCode</h1>

<p align="center">
  The <strong>OpenCode edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside OpenCode</strong>. Part of the Gald3r framework
...
This is the **OpenCode distribution** of Gald3r. The [`opencode/`](./opencode/) folder is a
complete, ready-to-deploy gald3r setup tuned for OpenCode -- drop its contents into your
...
cp -r gald3r_platform_opencode/opencode/. /path/to/your/project/
```

Then open the project in OpenCode and type `@g-status` to confirm gald3r loaded.

> The `opencode/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
